...
def create_initial_admin():
    with app.app_context():
        if not User.query.filter_by(username=per_info["username"]).first():
            hashed_password = generate_password_hash(per_info["password"])
            admin = User(username=per_info["username"], email=per_info["email"], password=hashed_password)
